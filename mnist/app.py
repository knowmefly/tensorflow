...
def restore_model(testPccArr):
	#创建默认图
	with tf.Graph().as_default() as tg:
		x = tf.placeholder(tf.float32, [None, forward.INPUT_NODE])
		y = forward.forward(x, None)
		preValue = tf.argmax(y, 1) #概率最大预测值

		#实现滑动平均，控制更新进度
		variable_averages = tf.train.ExponebtialMovingAverage(backward.MOVING_AVERAGE_DECAY)
		...
		with tf.Session() as sess:
			...
			if ckpt and ckpt.model_checkpoint_path:
				...
				preValue = sess.run(preValue, feed_dict={x:tetsPccArr})
				return preValue
			else:
